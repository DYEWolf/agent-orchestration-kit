...
not alternate conversation owners. For the lifecycle and evidence contract,
read `docs/agents/execution-policy.md` and `docs/agents/orca-execution.md`.
...
1. **Is context materially full, dominated by tool output, at a Campaign Issue
...
4. **Is the work moving to another harness, directory, or colleague?** Use
   `$handoff` and resume only from its durable artifact.
5. **Has the user explicitly authorized the next phase, or named an end-to-end
   flow that includes it?** Invoke that phase. Otherwise stop and present the
   result, waiting for direction.
6. **Is the current conversation no longer needed?** Clear or compact only after
   recording durable decisions or the checkpoint. Keep GitHub specs and
   implementation issues independent of session state.
