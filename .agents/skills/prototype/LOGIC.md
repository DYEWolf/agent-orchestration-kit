...
Before writing code, use the user-approved issue or Orca Task contract to state what state model and what question you're prototyping. Put one paragraph at the top of the demo (in a visible intro, not just a comment). A logic prototype that answers the wrong question is pure waste, so the coordinator must resolve ambiguity before dispatch; a worker escalates rather than choosing a question.
...
Return the artifact path and run instructions in the Task report. The coordinator presents or opens the demo for the user and records feedback; a worker does not contact the user directly. If the user wants new actions or a new scenario, the coordinator updates the issue/Task contract before more work begins.

### 5. Capture the answer and the prototype

Once the coordinator has recorded the answer, capture the prototype using the route in [SKILL](SKILL.md). The logic-specific mapping is unchanged: the validated reducer, machine, or function set lifts into the real module; the HTML shell is retained only at the coordinator-authorized durable location, with its issue pointer and integration ownership recorded.
