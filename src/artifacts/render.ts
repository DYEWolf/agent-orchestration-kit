...
const generatedHeader = '<!-- Generated by @dyewolf/agent-orchestration-kit. V1 does not update this file automatically. -->';
...
export function renderDesiredArtifacts(config: WorkflowConfig): DesiredArtifact[] {
  const artifacts: DesiredArtifact[] = [
    ...
    {
      path: '.agent-orchestration-kit/config.yaml',
      ownership: 'full',
      content: `# Generated by @dyewolf/agent-orchestration-kit. V1 does not update this file automatically.\n${stringifyYaml(config, { lineWidth: 0, aliasDuplicateObjects: false })}`,
    },
    {
  ...
  for (const skill of skillBundleCatalog.skills) {
    artifacts.push({
      ...
      content: [
        '---',
        `name: ${skill.name}`,
        'description: Load the canonical agent-orchestration-kit skill instructions.',
        '---',
        '',
        `# ${skill.name}`,
        '',
...
\`gh\` CLI at runtime. agent-orchestration-kit itself does not create Issues, commits, branches,
...
and hashes; first-party skills carry explicit agent-orchestration-kit authorship plus source
